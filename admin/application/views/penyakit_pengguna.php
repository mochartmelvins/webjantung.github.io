...
<?php
  $query = mysqli_query($koneksi, "SELECT * FROM tbpenyakit order by id_penyakit asc") or die (mysqli_error());
   if(mysqli_num_rows($query) == 0){
    echo "
	<tr>
  <td colspan='2' align='center'>Tidak Ada Data Yang Tersedia</td>
 </tr>
";
    }else{
									...
									<form id="demo-form2" method="post" action="" enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
										...
										<div class="item form-group">
											<div class="col-md-6 col-sm-6 offset-md-3">
												...
												<a href="dashboard.php?module=penyakit_pengguna"><button class="btn btn-primary" type="button">Batal</button></a>
...
if(isset($_POST['Simpan'])){
  ...
  if($querytambah) {
   ...
   echo"<script>location.href='$_SERVER[PHP_SELF]?module=penyakit_pengguna';</script>";
   
   
   } else{
   echo"<script>alert('Data Gagal di Input');location.href='$_SERVER[PHP_SELF]?module=penyakit_pengguna';</script>";
...
$id = $_GET['id'];
  ...
  if($queryhapus){
 # header('location: menu.php');
   echo"<script>alert('Data Berhasil di Hapus');location.href='$_SERVER[PHP_SELF]?module=penyakit_pengguna';</script>";
 }else{
 # echo "Upss Something wrong..";
  echo"<script>alert('Data Gagal di Hapus');location.href='$_SERVER[PHP_SELF]?module=penyakit_pengguna';</script>";
 }
...
$queryupdate = mysqli_query($koneksi, "UPDATE tbpenyakit SET 
  ...
  if($queryupdate) {
   ...
   echo"<script>alert('Data Gagal di Update');location.href='$_SERVER[PHP_SELF]?module=penyakit_pengguna&stt=edit&id=$id_penyakit';</script>";
