...
if(mysqli_num_rows($query) == 0){ 
 ...
 ?>
  <option value="<?php echo $r['id_konsultasi'] ?>"><?php 
  $tgl_konsul=$r['tgl_konsul'];
  $pisah=explode("-",$tgl_konsul);
  $tgl2=$pisah[2]."-".$pisah[1]."-".$pisah[0];
  
  echo "$no. Konsultasi $no Tanggal ($tgl2)"; ?></option>
 <?php
 $no++;
 endwhile;
...
if($hasil>$nilai){masuk_data_master
...
$id_konsultasi=$_GET['idk'];
?><hr>
						  <a href="<?php echo "media.php?module=hitung_user&stt=ulang&idk=$id_konsultasi";?>" class="menu"><button type="button" class="btn btn-primary mb-3">Selesai</button></a>
                            </div>
...
<?php
	...
	echo"<script>location.href='$_SERVER[PHP_SELF]?module=hasil_konsul';</script>";
	  }
?>
